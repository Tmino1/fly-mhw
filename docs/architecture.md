...
| Auto-skip the post-hunt "return to camp" wait | `DemoRecorder` optionally holds a second `VirtualGamepad` used only between episodes — one `BTN_SOUTH` tap the first time `quest.state==3` is seen, to skip the real-time wait instead of sitting through it. Opt-out via `record_hunt.py --no-skip-post-hunt-wait` | Requested to speed up back-to-back recording sessions. Deliberately kept off the actual demo-data path: this gamepad is never seen by `KeyboardActionReducer` (a separate object entirely), so it can't leak into an action label the way the human's real input does. **Unverified** that `BTN_SOUTH` is actually what dismisses that specific screen (only confirmed as the general confirm button elsewhere) — watch the first live use. |
